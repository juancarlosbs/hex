...
async moveNode(workspaceId: string, fromPath: string[], toParentPath: string[], index: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("move_node", { workspaceId, fromPath, toParentPath, index }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
