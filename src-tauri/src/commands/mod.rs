...
pub fn move_node(
    ...
    let dir = data_dir(&app)?;
    collection::move_node(&dir, &workspace_id, path, new_parent_path).map_err(|e| e.to_string())
}
...
pub fn duplicate_request(
    ...
    let dir = data_dir(&app)?;
    collection::duplicate_request(&dir, &workspace_id, path).map_err(|e| e.to_string())
}
